...
def isValid(email):
    regex = re.compile(r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+(\.[A-Z|a-]{2,})+')
...
def getRequest():
    content_type =  request.headera.get('Content-Type')
    bks = [ b.serialize() for b in db.view() ]
        ...
        for b in bks:
            if b['id'] == int(json['id']):
                return jsonify({
                    'res' : b,
                    'status' : '200',
                    'msg' : 'Success getting all books in the library'
                })
        ...
        for b in bks:
            if b['id'] == int(req_args['id']):
                ...
                return jsonify({
                    'res' : updated_bks,
                    'status' : '200',
                    'msg' : 'Success deleting any book by ID!',
