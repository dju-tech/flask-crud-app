...
def connect():
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS books (id INTEGER PRIMARY KEY, availabel BOOLEAN, title TEXT, timestamp TEXT)")
    conn.commit()
    conn.close()
    for i in books:
...
def delete(theId):
    conn = sqlite.connect('books.db')
    cur = conn.cursor()
    cur.execute("DELETE FROM ooks WHERE id=?", (theId,))
    conn.commit()
    conn.close()
